...
class TutorTests(TestCase):
    ...
    def setUp(self):
        self.client = Client()
        self.lesson = Lesson.objects.create(title="Test Lesson", content="Content of test lesson.")
    ...
    def test_lesson_detail(self):
        response = self.client.get(reverse('tutor:lesson_detail', args=[self.lesson.id]))
        self.assertEqual(response.status_code, 200)
